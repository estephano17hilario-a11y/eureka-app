  ...
  return `
    <div>
      ...
      <div class="figma-action-header">
        <h2 style="font-size:1.75rem; font-weight:800; color:#ffffff; letter-spacing:-0.02em;">Inicio</h2>
      ...
      <button class="figma-fab-gift" id="btn-fab-gift" title="Recompensas y rachas">
...
export function bindFigmaDeckListEvents(container: HTMLElement, callbacks: FigmaDeckListCallbacks): void {
  container.querySelectorAll('.figma-deck-row').forEach((row) => {
    row.addEventListener('click', () => {
      const id = (row as HTMLElement).dataset.deckId;
  ...
  container.querySelector('#btn-trash-decks')?.addEventListener('click', () => {
    const rootDecks = deckService.getRootDecks();
    if (rootDecks.length === 0) return;
    const names = rootDecks.map((d, i) => `${i + 1}. ${d.name}`).join('\n');
    const idx = prompt(`Elige el número del mazo a eliminar:\n${names}`);
    if (idx && !isNaN(Number(idx))) {
      const target = rootDecks[parseInt(idx, 10) - 1];
      if (target && confirm(`¿Eliminar mazo "${target.name}"?`)) {

...
export function bindFigmaSubdeckEvents(container: HTMLElement, deck: Deck, callbacks: FigmaSubdeckCallbacks): void {
  ...
  container.querySelector('#btn-create-child-subdeck')?.addEventListener('click', () => {
    const name = prompt(`Nombre del nuevo submazo en "${deck.name}":`);
    if (name && name.trim()) {

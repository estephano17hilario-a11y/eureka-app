...
export class FeynmanLlmService {
  ...
  public importMarkdownGuide(
    ...
    const levels = feynmanPedagogyService.parseFeynmanMarkdown(rawMarkdown);
    if (levels.length === 0) {
    ...
    // Inferir tema si no viene en el formulario
  ...
  public async generateGuide(
    ...
    const levels = feynmanPedagogyService.parseFeynmanMarkdown(generatedMarkdown);
  ...
  /**
   * Genera el código React 18 + TypeScript (TSX) con más de 500 líneas reales para cada nivel
   */
...
// TEMA: ${topic.toUpperCase()} | PASO: ${title.toUpperCase()}
// TECNOLOGÍA: REACT 18 + TYPESCRIPT (TSX) + HTML5 CANVAS + TAILWIND CSS
// ARQUITECTURA: MOTOR DE PARTÍCULAS, ONDAS RK4, AUDIO Y RETOS LÚDICOS (+500 LÍNEAS)
// ============================================================================
...
export function App() {
  ...
  const [frequency, setFrequency] = useState<number>(${20 + lvl * 4});
  const [damping, setDamping] = useState<number>(85);
  const [couplingFactor, setCouplingFactor] = useState<number>(70);
  const [visualMode, setVisualMode] = useState<'particles' | 'waves' | 'matrix'>('particles');
  const [pulseCount, setPulseCount] = useState<number>(0);
  const [activeTab, setActiveTab] = useState<'arena' | 'telemetry' | 'challenge' | 'theory'>('arena');
  
  // Estado del Reto Lúdico (Gamificación)
  ...
  const [telemetry, setTelemetry] = useState<SystemTelemetry>({
    ...
    meanEnergy: 75.0,
    lyapunovIndex: -0.09,
    conservationError: 0.002
  });

  // 2. Efecto de Sonido Web Audio
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      if (isRunning) {
        // Actualizar y dibujar ondas de choque (shockwaves)
        ...
        const speedMultiplier = (frequency / 40) * (couplingFactor / 50);

        for (let i = 0; i < particles.length; i++) {
          const p = particles[i];
          p.x += p.vx * speedMultiplier;
          p.y += p.vy * speedMultiplier;
          p.pulsePhase += 0.05 * (frequency / 25);
          ...
          // Conexiones de red entre partículas cercanas
          for (let j = i + 1; j < particles.length; j++) {
            ...
            if (pDist < 85) {
              const alpha = (1 - pDist / 85) * (couplingFactor / 100) * 0.45;
              ctx.beginPath();
              ctx.moveTo(p.x, p.y);
          ...
          // Dibujar partícula con aura pulsante
  ...
  }, [isRunning, frequency, damping, couplingFactor]);

  // 5. Motor de Telemetría Numérica y Verificación del Reto Lúdico
  useEffect(() => {
    if (!isRunning) return;
    ...
    const timer = setInterval(() => {
      setTelemetry((prev) => {
        ...
        const targetEntropy = Number((0.08 + (frequency / 100) * 0.35 * (1 - damping / 100)).toFixed(3));
        const targetStability = Math.min(100, Math.max(15, Math.round(100 - targetEntropy * 75)));
        const targetEnergy = Number((65 + Math.sin(nextCycles * 0.2) * (frequency * 0.2)).toFixed(1));
        const lyapunov = Number((-0.18 + (frequency / 100) * 0.3 - (damping / 100) * 0.2).toFixed(3));

        // Validación del Reto Lúdico
        if (targetStability >= 85 && !challengeCompleted) {
          setChallengeProgress((cp) => {
            const next = cp + 15;
            if (next >= 100) {
              setChallengeCompleted(true);
              setScore((s) => s + 75);
              playSoundEffect(880, 'triangle', 0.4);
              return 100;
            }
            return next;
          });
        ...
        return {
          ...
          stabilityScore: targetStability,
          meanEnergy: targetEnergy,
          lyapunovIndex: lyapunov,
          conservationError: Number((Math.abs(Math.cos(nextCycles)) * 0.004).toFixed(4))
        };
      });
    }, 300);

    return () => clearInterval(timer);
  }, [isRunning, frequency, damping, challengeCompleted, playSoundEffect]);

  // Manejador de Clic en el Canvas (Generar Onda de Choque Interactiva)
  const handleCanvasClick = (e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    playSoundEffect(440 + Math.random() * 300, 'triangle', 0.18);

    // Impulsar partículas hacia afuera desde el punto de clic
  ...
  // Inyección de Tormenta Caótica Lúdica
  const triggerChaosPulse = () => {
    setPulseCount((c) => c + 1);
    playSoundEffect(220, 'sawtooth', 0.35);
    ...
    // Inyectar 10 partículas de sobrecarga roja
    const newChaosParticles: Particle[] = [];
    for (let k = 0; k < 10; k++) {
      newChaosParticles.push({
    ...
    particlesRef.current = [...particlesRef.current, ...newChaosParticles].slice(-60);
  };

  return (
    <div className="p-4 md:p-6 bg-slate-950 text-slate-100 min-h-screen flex flex-col gap-6 font-sans">
      {/* CABECERA SCI-FI GAMIFICADA */}
      <header className="bg-gradient-to-r from-slate-900 via-sky-950/30 to-slate-900 border border-slate-800 p-5 rounded-2xl shadow-2xl backdrop-blur-xl flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <div className="flex items-center gap-2 mb-1">
            <span className="w-2.5 h-2.5 rounded-full bg-cyan-400 animate-ping"></span>
            <span className="text-[11px] font-black uppercase tracking-widest text-cyan-400">
              ⚡ SIMULADOR VISUAL LÚDICO • NIVEL ${lvl}
            </span>
          </div>
          ...
          <p className="text-xs text-slate-400 mt-0.5">
            Dominio Feynman: <strong className="text-slate-200">${topic}</strong>
          </p>
        </div>

        {/* HUD DE PUNTUACIÓN Y CONTROLES */}
        <div className="flex items-center gap-3 flex-wrap">
          <div className="bg-slate-950/90 px-4 py-2 rounded-xl border border-slate-800 flex items-center gap-3">
            <div>
              <span className="text-[9px] font-bold text-slate-400 uppercase">Puntuación</span>
              <p className="text-base font-black text-amber-400">{score} XP</p>
            </div>
            <div className="h-6 w-px bg-slate-800"></div>
            <div>
              <span className="text-[9px] font-bold text-slate-400 uppercase">Estabilidad</span>
              <p className={\`text-base font-black \${telemetry.stabilityScore >= 80 ? 'text-emerald-400' : 'text-rose-400'}\`}>
          ...
          <button
            onClick={triggerChaosPulse}
            className="px-4 py-2 bg-gradient-to-r from-rose-500 to-amber-500 hover:from-rose-400 hover:to-amber-400 text-slate-950 font-black rounded-xl text-xs uppercase tracking-wider transition-all shadow-lg shadow-rose-500/20 active:scale-95 flex items-center gap-1.5"
          >
            <span>💥</span><span>Inyectar Caos</span>
          </button>
        </div>
      </header>

      {/* ARENA GRÁFICA INTERACTIVA CANVAS (CANVAS 2D CON FÍSICA) */}
      <div className="bg-slate-900/90 border border-slate-800 rounded-2xl p-5 shadow-2xl flex flex-col gap-4">
        <div className="flex justify-between items-center flex-wrap gap-2">
          <div className="flex items-center gap-2">
            <span className="text-lg">🎨</span>
            <h2 className="text-xs font-black uppercase tracking-wider text-slate-200">
              Arena Gráfica de Partículas & Campo de Fuerzas
            </h2>
            <span className="text-[10px] text-cyan-400 bg-cyan-950/60 px-2 py-0.5 rounded-full border border-cyan-800/80 font-mono">
              Toca o haz clic en el canvas para emitir ondas
            </span>
          </div>

          {/* Barra de Integridad del Núcleo */}
          <div className="flex items-center gap-2">
            <span className="text-[11px] text-slate-400">Integridad:</span>
            <div className="w-32 bg-slate-950 rounded-full h-2.5 overflow-hidden border border-slate-800 p-0.5">
              <div
                className={\`h-full rounded-full transition-all duration-300 \${
                  telemetry.stabilityScore >= 80 ? 'bg-gradient-to-r from-emerald-500 to-cyan-400' : 'bg-gradient-to-r from-amber-500 to-rose-500 animate-pulse'
                }\`}
        ...
        {/* Canvas Interactivo */}
        <div className="relative rounded-xl overflow-hidden border border-slate-800 bg-slate-950/80 shadow-inner cursor-crosshair">
          <canvas
            ref={canvasRef}
            ...
            onMouseLeave={() => {
              mouseRef.current = { x: -1000, y: -1000, isDown: false };
            }}
            className="w-full h-[320px] block"
          />

          {/* Overlay de Logro de Reto */}
          {challengeCompleted && (
            <div className="absolute top-4 right-4 bg-emerald-950/90 border border-emerald-500/60 p-3 rounded-xl backdrop-blur-md flex items-center gap-3 animate-bounce">
              <span className="text-2xl">🏆</span>
              <div>
                <p className="text-xs font-black text-emerald-300 uppercase">¡Maestría de Nivel Lograda!</p>
                <p className="text-[10px] text-slate-300">Homeostasis de primeros principios verificada (+75 XP)</p>
              </div>
            </div>
          )}
        </div>

        {/* CONTROLES TÁCTILES LUMINOSOS Y CALIBRACIÓN */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-2">
          <div className="bg-slate-950/80 p-4 rounded-xl border border-slate-800/80">
            ...
            />
            <span className="text-[10px] text-slate-500 block mt-1">Aumenta la velocidad de las partículas y el flujo</span>
          </div>

          <div className="bg-slate-950/80 p-4 rounded-xl border border-slate-800/80">
            <div className="flex justify-between text-xs text-slate-300 mb-1.5 font-bold">
              <span>Amortiguamiento Elástico:</span>
              <span className="text-emerald-400 font-mono">{damping}%</span>
            </div>
            <input
              type="range"
              min="10"
            ...
            />
            <span className="text-[10px] text-slate-500 block mt-1">Absorbe el exceso de entropía y estabiliza el núcleo</span>
          </div>

          <div className="bg-slate-950/80 p-4 rounded-xl border border-slate-800/80">
            <div className="flex justify-between text-xs text-slate-300 mb-1.5 font-bold">
              <span>Acoplamiento Sináptico:</span>
              <span className="text-purple-400 font-mono">{couplingFactor}%</span>
            </div>
            <input
              type="range"
              min="10"
            ...
            />
            <span className="text-[10px] text-slate-500 block mt-1">Intensidad de las líneas de conexión entre nodos</span>
          </div>
        </div>

        {/* RETO LÚDICO DEL NIVEL (GAMIFICACIÓN) */}
        <div className="bg-gradient-to-r from-slate-950 via-indigo-950/30 to-slate-950 p-4 rounded-xl border border-indigo-500/30 flex flex-col md:flex-row justify-between items-center gap-4">
          <div>
            <div className="flex items-center gap-2 mb-1">
              <span className="text-sm">🎯</span>
              <h3 className="text-xs font-black uppercase tracking-wider text-indigo-300">
                Misión Gamificada: Calibración de Resonancia Estable
              </h3>
            </div>
            <p className="text-xs text-slate-300">
              Mantén la estabilidad del sistema por encima del <strong>85%</strong> ajustando el amortiguamiento para desbloquear la medalla de nivel.
            </p>
          </div>

          <div className="w-full md:w-56 space-y-1.5 flex-shrink-0">
            <div className="flex justify-between text-[11px] font-mono text-slate-400">
              <span>Progreso de Misión:</span>
              <span className="text-amber-400 font-bold">{challengeProgress}%</span>
            </div>
            <div className="w-full bg-slate-900 rounded-full h-2 overflow-hidden border border-slate-800">
              <div
                className="h-full bg-gradient-to-r from-indigo-500 via-cyan-400 to-amber-400 transition-all duration-300"
